...
func buildRegisterContactHeaderForInstance(contactURI, instanceID string) string {
	...
	contact := "<" + strings.TrimSpace(contactURI) + ">;+sip.instance=\"<" + instanceID + ">\";reg-id=1"
...
// registerContactURIForSecurityAssociation publishes the UE protected server
// endpoint negotiated by Security-Agree. The bootstrap REGISTER uses the
// ordinary SIP port; authenticated registrations must advertise port-s so
// terminating MESSAGE and INVITE requests enter the inbound IPsec flow.
